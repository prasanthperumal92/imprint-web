...
export class StatusComponent implements OnInit {
  ...
  getMyChart(type) {
    ...
    this.http.GET(`${CHART_TYPE_DATA}/${this.type}/${start}/${end}/${this.selectedEmployee.id}`).subscribe(res => {
      console.log(res);
  ...
  openModal(elem, data) {
    ...
    this.http.GET(`${CHART_TABLE_DATA}/${this.type}/${start}/${end}/${data.other}`).subscribe(res => {
      console.log(res);
      this.alert.showLoader(false);
      if (res && res[0]) {
