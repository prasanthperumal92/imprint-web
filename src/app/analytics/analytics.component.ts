...
export class AnalyticsComponent implements OnInit {
	...
	constructor(
		...
		this.salesChart = {
			id: 'salesChart',
			title: 'Sales Status',
			description: 'Number of sales completed in the selected time period.',
			xAxisName: 'Count',
			yAxisName: 'Activity',
			type: 'sales'
		};
		...
		this.productChart = {
			...
			description: 'Number of products completed in the selected time period.',
			xAxisName: 'Count',
			yAxisName: 'Activity',
			type: 'product'
		};
	...
	dateFilter(filter) {
		...
		if (typeof choosen.from.year === 'number') {
			const from = new Date(choosen.from.year, choosen.from.month, choosen.from.day);
